...
@onready var sound_toggle := $VBoxContainer/SoundSection/SoundToggle
@onready var sound_label := $VBoxContainer/SoundSection/SoundLabel
...
func update_ui():
	...
	theme_button.text = "Светлая тема" if is_light else "Тёмная тема"
	...
	ThemeManager.style_button(theme_button, Color("#DDA0DD"))
	sound_toggle.button_pressed = ThemeManager.sound_enabled
	_sound_toggle_label()
	sound_label.add_theme_color_override("font_color", ThemeManager.get_text())

func _sound_toggle_label():
	sound_toggle.text = "ON" if ThemeManager.sound_enabled else "OFF"
	var c = Color("#4CAF50") if ThemeManager.sound_enabled else Color("#999999")
	sound_toggle.add_theme_color_override("font_color", c)
	sound_toggle.add_theme_color_override("font_hover_color", c)
	sound_toggle.add_theme_color_override("font_pressed_color", c)
	sound_toggle.add_theme_color_override("font_focus_color", c)

func _on_sound_toggled(value: bool):
	ThemeManager.set_sound_enabled(value)

func _on_sound_changed(enabled: bool):
	_sound_toggle_label()
