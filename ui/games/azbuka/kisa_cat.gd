...
## собраны в kisa_sprite_frames.tres (idle / walk). Оба кадра walk нарисованы
## влево; при беге вправо спрайт отражается по горизонтали (flip_h) — так
## Киса поворачивается по ходу движения змейки. API совместим с прежней
## кодовой Кисой: set_kisa_moving(), set_facing().
...
var is_moving := false
## Направлен ли Киса вправо: при беге вправо спрайт отражается (flip_h).
...
func _ready() -> void:
	...
	var frame_height := SPRITE_FRAMES.get_frame_texture("walk", 0).get_height()
...
func _update_animation() -> void:
	...
	if is_moving:
		_sprite.play("walk")
		# Кадры walk нарисованы влево; вправо — зеркалим по ходу движения.
